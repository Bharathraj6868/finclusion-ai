...
@app.get("/health")
async def health():
    return {"status": "healthy", "components": {"credit_engine": "healthy", "fraud_detector": "healthy", "loan_service": "healthy"}}
    ...
    **Alternative Credit Scoring**
    ...
    return {
        ...
        "data_sources_used": [
            ...
            {"source": "Agricultural Data", "weight": 0.10, "available": random.random() > 0.5, "signal": "positive"}
        ],
        "score_factors": {
            "positive": [
...
async def micro_loan_application(payload: dict):
    ...
    **AI-Powered Micro-Loan Application**
    ...
    return {
        ...
        "monthly_installment_usd": round(approved_amount * (interest_rate/100/12) / (1 - (1 + interest_rate/100/12)**(-duration_months)), 2) if approved else None,
        ...
        "disbursement_method": random.choice(["M-Pesa", "UPI", "Bank Transfer", "Mobile Wallet"]),
...
async def detect_fraud(payload: dict):
    ...
    **Real-time Transaction Fraud Detection**
    ...
    """
    transaction_id = payload.get("transaction_id", f"TXN-{random.randint(100000, 999999)}")
    ...
    return {
        ...
        "risk_signals": [
            {"signal": "New device detected", "weight": 0.15, "triggered": random.random() > 0.6},
            {"signal": "Unusual time of day", "weight": 0.12, "triggered": random.random() > 0.7},
            {"signal": "Transaction velocity spike", "weight": 0.28, "triggered": random.random() > 0.8},
            {"signal": "Geographic anomaly", "weight": 0.22, "triggered": False},
            {"signal": "Amount pattern anomaly", "weight": 0.23, "triggered": random.random() > 0.75}
        ],
        "action": "Transaction processed normally" if not is_fraud else "Transaction blocked — user notified",
...
async def micro_insurance_quote(
    product: str = "crop",
    coverage_usd: float = 500,
    location: str = "Maharashtra,IN"
):
    """**Parametric Micro-Insurance Quotes** — Instant coverage for low-income users"""
    products = {
        "crop": {"premium_pct": 2.5, "trigger": "Rainfall below 60% of average", "payout_days": 3},
        "health": {"premium_pct": 3.8, "trigger": "Hospitalization > 24 hours", "payout_days": 7},
        "life": {"premium_pct": 1.2, "trigger": "Death or total disability", "payout_days": 14},
        "phone": {"premium_pct": 4.5, "trigger": "Device damage or theft", "payout_days": 2}
    }
    ...
    return {
        ...
        "payment_options": ["Monthly UPI", "Weekly Mobile Money", "Harvest-time payment (crop only)"]
    }
...
async def financial_impact():
    ...
    return {
        ...
        "sdg_alignment": ["SDG 1 (No Poverty)", "SDG 8 (Decent Work)", "SDG 10 (Reduced Inequalities)"]
    }
